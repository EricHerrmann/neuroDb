...
**Active focus:** Run the Knowledge Library augmentation manual gate (docs/testsPlans/manualTestPlan_knowledge_library_augmentation.md, KA1–KA4 + KB1–KB4) — includes the one-time `uv run python -m neurodb.cli.reconcile_fulltext` pass over the existing full-text corpus. Also run the remaining manual gates — TeX ingest (`docs/testsPlans/manualTestPlan_tex_ingest.md`), Phase 2b OA PDF + generic-HTML (`docs/testsPlans/manualTestPlan_citation_phase2b_pdf_html.md`, PB1–PB8 + LF1–LF3), and the combined Phase 2a + Learning Plans + Phase 1 gate (`docs/testsPlans/manualTestPlan_citation_phase2a_fulltext.md`, AG1–AG5, FT1–FT8, LP1–LP8). (Knowledge Library removal semantics signed off 2026-07-02 — gate archived in `docs/testsPlans/completedAndPassedTestPlans/manualTestPlan_knowledge_library_remove_references.md`. Literature-search provider expansion + source-provenance citations signed off 2026-06-27.)
**Next:** Pick up the literature source registry spec once the manual gates clear. **DEFERRED:** Phase 2b Docling high-fidelity PDF adapter — deferred to a future phase (PyMuPDF-first shipped); implement behind the `docling_convert` seam in `pdf_parser.py` when ready. Also deferred: drop the now-unused dead columns `research_questions.topic_id` and `study_notes.topic_id`/`concept_id` (retained to avoid a DuckDB table rebuild).
...
| **Active Plans / Specs** | |
| `docs/superpowers/specs/2026-07-08-knowledge-library-augmentation-design.md` | Knowledge Library augmentation design — fixes tutor not surfacing library content: (1) orchestrator-run deterministic library search on explicit natural-language request (search_full_text → summary fallback, mandatory full-content injection, visible searched-library line); (2) metadata backfill on acquisition (authors/abstract/year/DOI, external-by-DOI preferred, never clobber curated); (3) in-process `FullTextAcquired` event + reconciliation handler resyncing data_tier/authors/year/currency across papers + chunk + summary stores; (4) reference mining documented as separate future workstream; impl order 2→3→1; workstreams 1–3 implemented 2026-07-08 (manual gate pending), workstream 4 future |
| `docs/superpowers/plans/2026-07-08-knowledge-library-augmentation.md` | Knowledge Library augmentation implementation plan — 12 tasks (manual gate doc, metadata lookup client, backfill logic, event emitter, Chroma metadata updates, event_log + reconciliation handler, acquisition wiring, one-time reconcile CLI, library directive module, chat integration + prompt, frontend notice, verification/doc sync); ready to execute; implemented 2026-07-08, manual gate pending |
...
| `docs/superpowers/plans/2026-07-08-clickable-knowledge-library-citations.md` | Clickable Knowledge Library citations implementation plan — internal KL citation links, in-app chat navigation, Knowledge Library focus/scroll/highlight, visible paper IDs; implemented, manual gate pending |
...
| `docs/testsPlans/completedAndPassedTestPlans/manualTestPlan_knowledge_library_remove_references.md` | SIGNED OFF 2026-07-02 — Knowledge Library removal manual gate (T1–T5): hard delete for unreferenced papers, reference-blocker rationale with delete/replace choices, restore for legacy soft-removed papers |
...
| `docs/superpowers/specs/2026-06-26-literature-search-providers-design.md` | Literature-search providers design — base-class + registry refactor of the live search layer (realizes the 2026-06-02 source-registry intent) + 4 new free providers (OpenAlex, Europe PMC, Crossref, bioRxiv); env toggles + polite-pool email; concurrent fan-out; enrich-merge + citation ranking; JSON provider-counts audit column; implemented 2026-06-26 |
| `docs/superpowers/plans/2026-06-26-literature-search-providers.md` | Literature-search providers implementation plan — 9 TDD tasks (base provider, migration 026, port existing 3, merge, registry, fan-out client, 4 new providers, integration, manual gate); implemented 2026-06-26 |
| `docs/superpowers/specs/2026-06-27-citation-provenance-design.md` | Source-provenance citations design — shared CITATION_PROVENANCE_RULE for NeuroTutor + NeuroResearch: `(Knowledge Library · metadata/abstract/full text)` for library papers, Markdown URL link for non-library; data_tier_label helper; grouping bundle exposes data_tier + url; implemented 2026-06-27 |
| `docs/superpowers/plans/2026-06-27-citation-provenance.md` | Source-provenance citations implementation plan — 4 TDD tasks (shared rule + helper, both-agent prompt wiring, grouping bundle fields, manual gate); implemented 2026-06-27 |
| `docs/testsPlans/completedAndPassedTestPlans/manualTestPlan_literature_search_providers.md` | SIGNED OFF 2026-06-27 — live multi-provider literature search (OpenAlex/Europe PMC/Crossref/bioRxiv; semantic_scholar disabled) connectivity + merge/toggle/audit, and source-provenance citation rendering (Step 4) |
