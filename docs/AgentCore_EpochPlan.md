...
| Chat history — topic inference + full summary storage | Phase A of a two-phase chat history improvement. Current `inferred_topic` is the raw first user message (200 chars); `summary_preview` is truncated at 200 chars. Changes needed: (1) `SessionManager.end_session` extracts a 5–10 word LLM-generated topic label and writes it back to `inferred_topic`; (2) add `summary_text` TEXT column to `ChatSession` for a full paragraph summary alongside the existing preview; (3) DB migration. UI search and display handled in UI epoch. Phase B (compaction/merge) is a separate future item. |
