...
|--------|----------|-------|----------|-------------|----------|
| LOG-062 | topic-semantic-suggestions | Research | Medium | Question topic/concept suggestion uses literal substring matching against existing taxonomy and ignores topic_context; off-domain questions get zero suggestions. Implement spec's semantic agent-based suggestion. | Feature enhancement |
...
|--------|------|----------|-------|----------|-------------|---------|
| LOG-062 | 2026-06-01 | topic-semantic-suggestions | Research | Medium | Feature enhancement: question→topic/concept suggestion should be semantic, not literal. Current `extract_question_topics` (topic_store.py:279) only matches when an existing active topic name is a literal substring of the question text, never creates new topics, and is fed only `body.question` — the user-entered `topic_context` is stored but never used for suggestion. Spec (`2026-06-01-research-question-phase1-design.md:89-108`) intends an LLM agent that semantically suggests relevant existing topics/concepts. Found during Research Question Phase 1 T1: an off-domain question (AI/memory) against a stroke/plasticity taxonomy (27 topics, 0 concepts) produced 0 suggestions — correct per current code, but no chips appear. Enhancement: implement semantic suggestion and incorporate topic_context. NOTE: the suggestion *chips* render correctly below the question row (`ResearchPanel.tsx:292-312`); the chips seen above the list are the T4 topic filter bar, not suggestions. | Research Question Phase 1 manual test T1 |
