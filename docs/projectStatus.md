...
**Active focus:** Knowledge Library local-file source — implemented and pending manual verification. Adds a drop-folder library (`knowledge_library_files/`, gitignored), `GET /api/knowledge-library/library-files`, an acquire `source_path` field routed by extension (`.pdf`/`.html` → PyMuPDF/trafilatura + 2b gate; `.txt`/`.md` → user-supplied-text synchronous), a `library_store` path-traversal guard, and a file picker in the acquire UI. Built on top of the complete Phase 2b stack. Backend **997 tests green**; frontend **119 tests green** and build clean. Manual gate: `docs/testsPlans/manualTestPlan_citation_phase2b_pdf_html.md` (PB1–PB8 + new LF1–LF3 local-file cases). Phase 2b itself (OA PDF + generic-HTML full text) remains pending manual verification alongside this. The combined Phase 2a + Learning Plans + Phase 1 gate (`docs/testsPlans/manualTestPlan_citation_phase2a_fulltext.md`) also remains pending.
