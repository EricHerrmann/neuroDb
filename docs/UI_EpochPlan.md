...
| Future | Chat history compaction + knowledge surface (Phase B) | After Phase A ships and session volume makes pruning/merging feel necessary: merge multiple sessions under one topic into a consolidated summary entry; archive or delete originals; edit topic labels; promote a topic group to a Knowledge Library–style curated record. Design question to resolve at that time: does this stay in Study Log or move to a dedicated History panel? Requires re-examining `ChatSession` schema and whether session transcripts (not just summaries) need to be stored. |
