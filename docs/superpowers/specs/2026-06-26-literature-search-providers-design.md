...
- **Functional (real server/DB wiring, not duplicating units):** run a real neuroscience query (e.g., "synaptic plasticity LTP") through the live `search_literature` path against the running FastAPI API (and the React research workbench where it surfaces results). Verify merged cross-provider results, citation-ranked ordering, `sources` provenance showing multiple contributors, and graceful behavior when one provider is disabled. (Streamlit is deprecated and is not a target surface for this design.)
