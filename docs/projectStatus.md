...
**Active focus:** Unified Groupings Phase 3a complete — the question workflow is cut over to the unified engine with an LLM-based semantic/proposal matcher (`agent.extract.groupings`), `/api/research/groupings` routes, the proposal lifecycle, and migration 018 hierarchy seed; LOG-062 closed server-side. 829 tests green.
**Next:** Unified Groupings Phase 3b (UI: proposal "new" chips, hierarchy/curation view, repoint the filter query to `GET /api/research/groupings`).
...
| Research | `src/neurodb/research/` | Scaffolded (LT-3); Phase 3 claims/evidence/gaps complete; Phase 2/3 manual signed off 2026-05-21; Phase 4 grounded/contextual behavior signed off 2026-05-21; lifecycle UI gaps from LOG-037, LOG-048, and LOG-061 resolved in Phase 5b; queue/tool gaps from LOG-045 and LOG-053 resolved; dataset usefulness surfaced to agents in Memory Refocus Completion; Unified Groupings Phase 1, 2, and 3a complete (semantic/proposal matcher, /groupings routes, migration 018 seed; LOG-062 closed) | Unified Groupings Phase 3b (UI: proposal chips, hierarchy view, filter repoint) |
...
| `docs/superpowers/plans/2026-06-01-groupings-phase3b-ui.md` | Groupings Phase 3b UI plan — filter repoint to /groupings, proposal "new" chips, topic hierarchy curation view; 5 tasks (not yet implemented) |
