...
- [ ] Agent Chat tab visible as fourth nav option
- [ ] Dataset count question answered with correct number from DB
- [ ] Semantic search returns real dataset IDs from ChromaDB
- [ ] Study notes query returns tags from DB
- [ ] Agent can tag a dataset; tag appears in Study Log
- [ ] Agent does not fabricate details for unknown dataset IDs
