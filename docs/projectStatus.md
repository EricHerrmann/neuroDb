...
**Active focus:** No active phase. Unified Groupings Phases 1–5 and Research Question Phase 1 are complete and signed off as of 2026-06-04. The final Phase 5 gate dropped legacy `topics`/`concepts` tables and their six join tables (migration 021), verified restart safety, and smoke-tested research-question, tutor, Knowledge Library, and Study Log workflows on the post-drop DB.
**Next:** Pick up a now-unblocked deferred spec (literature source registry or learning plans). Deferred: drop the now-unused dead columns `research_questions.topic_id` and `study_notes.topic_id`/`concept_id` (retained to avoid a DuckDB table rebuild).
...
| **Active Plans / Specs** | |
| _None_ | No active phase plan. Groupings and Research Question Phase 1 are complete. |
| **Deferred / Upcoming** | |
